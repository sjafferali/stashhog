...
from sqlalchemy import Column, String, Boolean, Text, DateTime, JSON, ForeignKey, Index, Integer, Float
...
class Scene(BaseModel):
    ...
    # Foreign keys
    studio_id = Column(String, ForeignKey("studio.id", ondelete="SET NULL"), nullable=True, index=True)
    ...
    def to_dict(self, exclude: set = None) -> dict:
        ...
        if hasattr(self, "performers"):
            data["performers"] = [
                {"id": p.id, "name": p.name} for p in self.performers
            ]
        
        if hasattr(self, "tags"):
            data["tags"] = [
                {"id": t.id, "name": t.name} for t in self.tags
            ]
