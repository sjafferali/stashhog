...
class Paginator(Generic[T]):
    ...
    def __init__(
        self, query: Select, page: int = 1, size: int = 20, max_size: int = 100
    ):
        """
    ...
    async def paginate(
        self, session: AsyncSession, transformer: Optional[Callable[[Any], T]] = None
    ) -> PaginatedResponse[T]:
        """
...
def apply_sorting(
    query: Select, sort: Optional[str], sort_fields: Dict[str, Any]
) -> Select:
    """
...
def parse_filters(filters: Dict[str, Any], allowed_fields: List[str]) -> Dict[str, Any]:
    """
