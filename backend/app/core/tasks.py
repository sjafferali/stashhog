...
class TaskQueue:
    ...
    async def _notify_callbacks(self, task_id: str, event: str):
        """
...
def background_task(name: Optional[str] = None):
    ...
    def decorator(func: Callable) -> Callable:
        ...
        wrapper.submit = wrapper  # Alias for clarity
        wrapper.original = func  # Keep reference to original
...
@background_task(name="sync_scenes")
async def sync_scenes_task(filter_params: Dict[str, Any], limit: Optional[int] = None):
    """Background task to sync scenes from Stash."""
    logger.info(f"Starting scene sync with params: {filter_params}, limit: {limit}")
    
    # TODO: Implement actual sync logic
    # This would involve:
    # 1. Connect to Stash API
    # 2. Fetch scenes based on filter
    # 3. Update database
    # 4. Update progress periodically
    
    await asyncio.sleep(5)  # Simulate work
    
    return {"synced": 10, "updated": 5, "errors": 0}


@background_task(name="analyze_scene")
async def analyze_scene_task(scene_id: str, options: Dict[str, Any]):
    """Background task to analyze a scene."""
    logger.info(f"Starting scene analysis for {scene_id} with options: {options}")
