...
class DetailsGenerator:
    ...
    async def enhance_description(
        ...
        if result.value and self._calculate_similarity(current_clean, result.value) > 0.8:
    ...
    def create_basic_description(
        ...
        if performers:
            ...
            if names:
                if len(names) == 1:
                    parts.append(f"featuring {names[0]}")
                elif len(names) == 2:
                    parts.append(f"featuring {names[0]} and {names[1]}")
                else:
                    parts.append(f"featuring {', '.join(names[:-1])}, and {names[-1]}")
