...
class BatchProcessor:
        ...
        try:
            ...
            for scene in batch:
                scene_dict = {
                    ...
                    "performers": [{"id": p.id, "name": p.name} for p in scene.performers],
                    "tags": [{"id": t.id, "name": t.name} for t in scene.tags],
                    "studio": {"id": scene.studio.id, "name": scene.studio.name} if scene.studio else None
                }
                batch_data.append(scene_dict)
            
            # Analyze the batch
            return await analyzer(batch_data)
    ...
    async def _process_batch_with_progress(
        ...
        async with self._semaphore:
            ...
            try:
                logger.debug(f"Processing batch {batch_idx + 1} with {len(batch)} scenes")
