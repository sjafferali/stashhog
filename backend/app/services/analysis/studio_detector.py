...
from .prompts import STUDIO_DETECTION_PROMPT, STUDIO_PATTERN_PROMPT
...
class StudioDetector:
    ...
    def _load_patterns(self) -> Dict[str, re.Pattern]:
        ...
        patterns = {
            ...
            "Lucas Entertainment": re.compile(r'lucas[\s_-]?entertainment|lucas[\s_-]?men', re.IGNORECASE),
            "Corbin Fisher": re.compile(r'corbin[\s_-]?fisher|cf[\s_-]?\d+', re.IGNORECASE),
            "Bel Ami": re.compile(r'bel[\s_-]?ami|belami', re.IGNORECASE),
            "Falcon Studios": re.compile(r'falcon[\s_-]?studios?', re.IGNORECASE),
            "Raging Stallion": re.compile(r'raging[\s_-]?stallion|rs[\s_-]?\d+', re.IGNORECASE),
            "Hot House": re.compile(r'hot[\s_-]?house', re.IGNORECASE),
            "Naked Sword": re.compile(r'naked[\s_-]?sword|nakedsword', re.IGNORECASE),
            "Treasure Island Media": re.compile(r'treasure[\s_-]?island|tim[\s_-]?\d+', re.IGNORECASE),
            "Raw Fuck Club": re.compile(r'raw[\s_-]?fuck[\s_-]?club|rfc', re.IGNORECASE),
            "Sketchy Sex": re.compile(r'sketchy[\s_-]?sex', re.IGNORECASE),
            "Breed Me Raw": re.compile(r'breed[\s_-]?me[\s_-]?raw', re.IGNORECASE),
            "Bareback That Hole": re.compile(r'bareback[\s_-]?that[\s_-]?hole|bbth', re.IGNORECASE),
            ...
            "JustForFans": re.compile(r'justforfans|just[\s_-]?for[\s_-]?fans|jff', re.IGNORECASE),
            ...
            for part in path_parts[:-1]:  # Exclude filename
                if pattern.search(part):
                    ...
                    return DetectionResult(
                        ...
                        confidence=confidence,
                        source="pattern",
                        metadata={"pattern": pattern.pattern, "matched_in": "directory"}
                    )
