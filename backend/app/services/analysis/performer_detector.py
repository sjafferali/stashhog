...
from .prompts import PERFORMER_DETECTION_PROMPT, PERFORMER_ALIAS_PROMPT
...
class PerformerDetector:
    ...
    SEPARATORS = [' and ', ' & ', ', ', ' - ', '_', ' with ', ' feat ', ' ft ', ' featuring ']
    
    # Words to ignore when extracting names
    IGNORE_WORDS = {
        'scene', 'part', 'episode', 'ep', 'video', 'clip', 'raw', 'bareback',
        'bb', 'hd', '1080p', '720p', '4k', 'uhd', 'mp4', 'avi', 'mkv',
        'wmv', 'mov', 'fuck', 'fucks', 'fucking', 'sex', 'anal', 'oral',
        'blowjob', 'bj', 'handjob', 'hj', 'cumshot', 'creampie', 'compilation'
    }
    
    def __init__(self):
        """Initialize performer detector."""
        self._performer_cache = {}
    ...
    def _extract_names_from_string(self, text: str) -> List[str]:
        ...
        # If no separators found, try to extract from the whole string
        if not names:
