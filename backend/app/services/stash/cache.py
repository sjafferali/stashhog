...
class StashEntityCache:
    ...
    def set_performers(self, performers: List[Dict]) -> None:
        ...
        for performer in performers:
            self.cache.set(f"entities:performers:id:{performer['id']}", performer, self.entity_ttl)
            self.cache.set(f"entities:performers:name:{performer['name'].lower()}", performer, self.entity_ttl)
    ...
    def set_tags(self, tags: List[Dict]) -> None:
        ...
        for tag in tags:
            ...
            self.cache.set(f"entities:tags:name:{tag['name'].lower()}", tag, self.entity_ttl)
    ...
    def set_studios(self, studios: List[Dict]) -> None:
        ...
        for studio in studios:
            self.cache.set(f"entities:studios:id:{studio['id']}", studio, self.entity_ttl)
            self.cache.set(f"entities:studios:name:{studio['name'].lower()}", studio, self.entity_ttl)
