...
class SyncStrategy(ABC):
    ...
    @abstractmethod
    async def should_sync(self, remote_data: Dict[str, Any], local_entity: Optional[Any]) -> bool:
...
class FullSyncStrategy(SyncStrategy):
    ...
    def _merge_scene_data(self, scene: Scene, remote_data: Dict[str, Any]) -> Scene:
        ...
        scene.details = remote_data.get("details")
        scene.url = remote_data.get("url")
        scene.date = remote_data.get("date")
        scene.rating = remote_data.get("rating")
    ...
    def _merge_entity_data(self, entity: Any, remote_data: Dict[str, Any]) -> Any:
        entity.name = remote_data.get("name", "")
        if hasattr(entity, "aliases"):
            entity.aliases = remote_data.get("aliases")
...
class SmartSyncStrategy(SyncStrategy):
    ...
    async def _smart_merge_scene(self, scene: Scene, remote_data: Dict[str, Any]) -> Scene:
        ...
        # Update checksum
        scene.content_checksum = self._calculate_checksum(remote_data)
        scene.updated_at = datetime.utcnow()
    ...
    def _calculate_checksum(self, data: Dict[str, Any]) -> str:
        ...
        important_fields = {
            "title", "details", "url", "date", "rating",
            "organized", "file", "performers", "tags", "studio"
        }
