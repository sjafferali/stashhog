...
from app.models import ScheduledTask, Job, JobType, JobStatus
...
class SyncScheduler:
    ...
    async def _run_full_sync(self, force: bool = False):
        ...
        try:
            with SessionLocal() as db:
                ...
                job = Job(
                    ...
                    parameters={"force": force, "scheduled": True},
    ...
    async def _run_incremental_sync(self):
        ...
        try:
            with SessionLocal() as db:
                ...
                job = Job(
                    id=job_id,
                    type=JobType.SCENE_SYNC,
                    status=JobStatus.IN_PROGRESS,
                    parameters={"incremental": True, "scheduled": True},
                    created_at=datetime.utcnow()
                )
                db.add(job)
                db.commit()
