...
from app.models import Scene, Performer, Tag, Studio, scene_performers, scene_tags
...
class SceneSyncHandler:
    ...
    async def sync_scene(
        ...
        scene = await self.strategy.merge_data(scene, stash_scene)
    ...
    async def sync_scene_batch(
        ...
        for scene_data in stash_scenes:
            try:
                ...
                # Apply sync strategy
                scene = await self.strategy.merge_data(scene, scene_data)
                
                # Sync relationships using pre-fetched entities
                await self._sync_scene_relationships_batch(
                    scene, scene_data, db,
                    performers_map, tags_map, studios_map
                )
    ...
    async def _sync_scene_performers(
        ...
        for performer_data in performers_data:
            ...
            performer = db.query(Performer).filter(Performer.stash_id == performer_id).first()
    ...
    def _fetch_entities_map(
        ...
        entities = db.query(model_class).filter(
            model_class.stash_id.in_(entity_ids)
        ).all()
