...
from .models import SyncResult, SyncError
...
class SyncProgress:
    ...
    async def _send_progress_update(self):
        ...
        try:
            await self._websocket_manager.broadcast(progress_data)
        except Exception as e:
            logger.error(f"Failed to send WebSocket update: {str(e)}")
    
    async def _send_detailed_update(self, entity_type: str, entity_id: str, action: str):
        ...
        try:
            await self._websocket_manager.broadcast(detail_data)
        except Exception as e:
            logger.error(f"Failed to send detailed WebSocket update: {str(e)}")
    
    async def _send_completion_update(self, result: SyncResult):
        ...
        try:
            await self._websocket_manager.broadcast(completion_data)
        except Exception as e:
