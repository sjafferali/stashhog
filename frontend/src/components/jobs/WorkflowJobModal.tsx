...
import styles from './WorkflowJobModal.module.scss';

const { Text, Title } = Typography;
const { Step } = Steps;

export interface WorkflowJobModalProps {
  job: Job;
  visible: boolean;
  onClose: () => void;
  onCancel?: () => void;
...
export const WorkflowJobModal: React.FC<WorkflowJobModalProps> = ({
  job,
  visible,
  onClose,
  onCancel,
    ...
    <Modal
      title={
        <div className={styles.modalHeader}>
          <Space>
            {getStatusIcon()}
            <Title level={4}>{job.name || 'Process New Scenes Workflow'}</Title>
          </Space>
