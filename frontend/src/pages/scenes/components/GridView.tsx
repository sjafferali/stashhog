...
import { Card, Col, Row, Image, Tag, Space, Checkbox, Typography, Tooltip } from 'antd';
import { 
  ClockCircleOutlined, 
  FolderOutlined, 
  CalendarOutlined,
...
export const GridView: React.FC<GridViewProps> = ({ scenes, onSceneSelect }) => {
  const { gridSize, selectedScenes, toggleSceneSelection } = useScenesStore();
  const colSpan = GRID_COLS[gridSize];

  const handleCardClick = (scene: Scene, e: React.MouseEvent) => {
  ...
  const handleCheckboxClick = (sceneId: string, e: React.MouseEvent) => {
    e.stopPropagation();
    toggleSceneSelection(sceneId);
  };

  const sceneCards = useMemo(() => scenes.map((scene) => {
              ...
              <Space direction="vertical" size="small" style={{ width: '100%' }}>
                ...
                <Space size="small" wrap>
                  {scene.performers && scene.performers.length > 0 && (
                    <Tooltip title={scene.performers.map(p => p.name).join(', ')}>
                      <Tag color="pink">
                        {scene.performers.length} performer{scene.performers.length > 1 ? 's' : ''}
                      </Tag>
                    </Tooltip>
                  )}
                  
                  {scene.tags && scene.tags.length > 0 && (
                    <Tooltip title={scene.tags.map(t => t.name).join(', ')}>
                      <Tag color="green">
                        {scene.tags.length} tag{scene.tags.length > 1 ? 's' : ''}
                      </Tag>
                    </Tooltip>
                  )}
                </Space>
                ...
                <Space size="small" className={styles.metadata}>
                  ...
                  {scene.size && (
                    <Tooltip title={`File size: ${formatFileSize(scene.size)}`}>
                      <FolderOutlined />
                    </Tooltip>
                  )}
  ...
  }), [scenes, colSpan, selectedScenes, onSceneSelect, toggleSceneSelection]);
