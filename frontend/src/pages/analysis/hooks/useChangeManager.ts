...
export function useChangeManager(
  ...
  const [sort, setSort] = useState<ChangeSort>({
    field: 'confidence',
    order: 'desc',
  });
  ...
  const addToHistory = useCallback(
    (entry: Omit<ChangeHistory, 'timestamp'>) => {
  ...
  const filteredChanges = useMemo(() => {
    return changes.filter((change) => {
      ...
      if (filter.status) {
        ...
        if (filter.status === 'pending' && (change.accepted || change.rejected))
          return false;
      }
      if (filter.confidenceMin && change.confidence < filter.confidenceMin)
        return false;
      if (filter.confidenceMax && change.confidence > filter.confidenceMax)
        return false;
  ...
  const acceptByFilter = async () => {
    ...
    void message.success(`Accepted ${toAccept.length} changes`);
  };
  ...
  const rejectByFilter = async () => {
    ...
    void message.success(`Rejected ${toReject.length} changes`);
  };

  // Accept by confidence threshold
  const acceptByConfidence = async (threshold: number) => {
    const toAccept = changes.filter(
      (c) => !c.accepted && !c.rejected && c.confidence >= threshold
    );
    ...
    void message.success(`Accepted ${toAccept.length} high-confidence changes`);
  };

  // Accept by field
  const acceptByField = async (field: string) => {
    const toAccept = changes.filter(
      (c) => !c.accepted && !c.rejected && c.field === field
    );
    ...
    void message.success(`Accepted ${toAccept.length} ${field} changes`);
  };
  ...
  const undo = () => {
    ...
    if (action.action === 'accept' || action.action === 'reject') {
      const update =
        typeof action.previousValue === 'object' &&
        action.previousValue !== null
          ? (action.previousValue as Partial<ProposedChange>)
          : {};
      onChangeUpdate(action.changeId, update);
    } else if (action.action === 'edit') {
      onChangeUpdate(action.changeId, { proposedValue: action.previousValue });
    }

    setHistoryIndex(historyIndex - 1);
    void message.info('Action undone');
  };
  ...
  const redo = () => {
    ...
    setHistoryIndex(historyIndex + 1);
    void message.info('Action redone');
  };
  ...
  const exportChanges = (format: 'json' | 'csv' | 'markdown') => {
    ...
    if (format === 'json') {
      ...
      // Group by scene
      const byScene = acceptedChanges.reduce(
        (acc, change) => {
          const sceneId = change.sceneId || 'unknown';
          if (!acc[sceneId]) acc[sceneId] = [];
          acc[sceneId].push(change);
          return acc;
        },
    ...
    void message.success(`Exported ${acceptedChanges.length} changes`);
  };

  // Apply changes to Stash
  const applyChanges = async (selectedOnly?: string[]) => {
    const toApply = changes.filter(
      (c) => c.accepted && (!selectedOnly || selectedOnly.includes(c.id))
    );

    if (toApply.length === 0) {
      void message.warning('No accepted changes to apply');
      return;
    }
    ...
    try {
      ...
      if (response.data.job_id) {
        void message.info('Changes are being applied in the background');
      ...
      void message.error('Failed to apply changes');
    }
  };
