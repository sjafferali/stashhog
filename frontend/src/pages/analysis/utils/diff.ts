...
export function textDiff(
  ...
  const withLineNumbers =
    mode === 'lines' ? addLineNumbers(diffChanges) : diffChanges;
...
export function arrayDiff<T>(
  ...
  const oldMap = new Map(
    oldArray.map((item, index) => [getKey(item), { item, index }])
  );
  const newMap = new Map(
    newArray.map((item, index) => [getKey(item), { item, index }])
  );
...
export function smartDiff(
  oldValue: unknown,
  newValue: unknown
): DiffResult | ArrayDiff<unknown> | ObjectDiff {
