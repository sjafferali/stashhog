...
const PlanDetail: React.FC = () => {
  ...
  const changeManager = useChangeManager(
    ...
    (changeId, update) => {
      ...
      if (update.accepted !== undefined) {
        update.accepted ? acceptChange(changeId) : rejectChange(changeId)
      }
      if (update.proposedValue !== undefined) {
        updateChange(changeId, update.proposedValue)
      }
    }
  )
  ...
  const bulkActionsMenu = (
    <Menu>
      <Menu.Item 
        ...
        onClick={() => {
          acceptAllChanges()
          message.success('All changes accepted')
        }}
      >
        Accept All Changes
      </Menu.Item>
      <Menu.Item 
        ...
        onClick={() => {
          rejectAllChanges()
          message.success('All changes rejected')
        }}
      >
        Reject All Changes
      </Menu.Item>
      ...
      <Menu.Item 
        ...
        onClick={() => {
          Modal.confirm({
            ...
            onOk: () => changeManager.acceptByConfidence(confidenceThreshold)
          })
        }}
      >
        Accept by Confidence
      </Menu.Item>
      <Menu.SubMenu key="accept-field" title="Accept by Field" icon={<FilterOutlined />}>
        <Menu.Item onClick={() => changeManager.acceptByField('title')}>Title</Menu.Item>
        <Menu.Item onClick={() => changeManager.acceptByField('performers')}>Performers</Menu.Item>
        <Menu.Item onClick={() => changeManager.acceptByField('tags')}>Tags</Menu.Item>
        <Menu.Item onClick={() => changeManager.acceptByField('studio')}>Studio</Menu.Item>
        <Menu.Item onClick={() => changeManager.acceptByField('details')}>Details</Menu.Item>
      </Menu.SubMenu>
    </Menu>
  )
  ...
  const exportMenu = (
    <Menu>
      <Menu.Item 
        key="json"
        onClick={() => changeManager.exportChanges('json')}
      >
        Export as JSON
      </Menu.Item>
      <Menu.Item 
        key="csv"
        onClick={() => changeManager.exportChanges('csv')}
      >
        Export as CSV
      </Menu.Item>
      <Menu.Item 
        key="markdown"
        onClick={() => changeManager.exportChanges('markdown')}
      >
        Export as Markdown
      </Menu.Item>
    </Menu>
    ...
    <div>
      ...
      <Card size="small" style={{ marginBottom: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <Progress 
            percent={((stats.acceptedChanges + stats.rejectedChanges) / stats.totalChanges) * 100}
            strokeColor={{
              '0%': '#108ee9',
              '100%': '#87d068',
            }}
          ...
          <Card title="Plan Summary" size="small">
            <PlanSummary
              plan={{
                ...
                extract_performers: plan.metadata.options?.detect_performers || false,
              ...
              statistics={{
                ...
                fieldBreakdown: {
                  ...
                  performers: allChanges.filter(c => c.field === 'performers').length,
                  tags: allChanges.filter(c => c.field === 'tags').length,
                  studio: allChanges.filter(c => c.field === 'studio').length,
                  details: allChanges.filter(c => c.field === 'details').length,
