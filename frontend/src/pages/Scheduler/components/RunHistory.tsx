...
import { Table, Tag, Space, Typography, Select, Button, Tooltip, Progress, Empty, Card, Row, Col } from 'antd';
import { 
  CheckCircleOutlined, 
  CloseCircleOutlined, 
  SyncOutlined,
...
const RunHistory: React.FC<RunHistoryProps> = ({ scheduleId, onScheduleSelect, schedules }) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      sorter: (a: ScheduleRun, b: ScheduleRun) => dayjs(a.started_at).unix() - dayjs(b.started_at).unix(),
    ...
    },
    {
      ...
      sorter: (a: ScheduleRun, b: ScheduleRun) => (a.duration || 0) - (b.duration || 0),
    },
    {
      title: 'Status',
      dataIndex: 'status',
      ...
      onFilter: (value: string | number | boolean, record: ScheduleRun) => record.status === value,
    },
    {
      title: 'Job ID',
      dataIndex: 'job_id',
      key: 'job_id',
      render: (job_id?: string) => job_id ? (
        <Tooltip title="View job details">
          <Button type="link" size="small">{job_id.substring(0, 8)}...</Button>
        </Tooltip>
      ) : '-',
    },
    {
    ...
    <Space direction="vertical" style={{ width: '100%' }} size="large">
      ...
      <Table
        ...
        locale={{
          emptyText: (
            <Empty
              description={scheduleId ? "No runs for this schedule yet" : "No runs yet"}
            />
          ),
        }}
      />
